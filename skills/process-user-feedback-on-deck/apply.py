...
Reads the edit-output file written by edit-card-batch, drops any note_ids the user
skipped in Confirm, applies the rest via update_note_fields_batch, and prints the
per-note results as JSON on stdout for the orchestrator's Report step.

Clearing `user_feedback` is what "applied" means, so this script sets it on every
surviving entry itself — edit-card-batch never needs to emit it.
